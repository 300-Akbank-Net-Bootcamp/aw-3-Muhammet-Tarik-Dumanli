...
public class AccountTransactionsController : ControllerBase
{
    ...
    [HttpGet("{Amount}/{Description}")]
    public async Task<ApiResponse<List<AccountTransactionResponse>>> Get(decimal Amount, string Description)
    {
        var operation = new GetAccountTransactionByParameterQuery(Amount, Description);
        var result = await mediator.Send(operation);
        return result;
    }
