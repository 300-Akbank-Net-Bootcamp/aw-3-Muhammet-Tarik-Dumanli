...
public class EftTransactionsController : ControllerBase
{
    ...
    public async Task<ApiResponse<List<EftTransactionResponse>>> Get(decimal Amount, string Description)
    {
        var operation = new GetEftTransactionByParameterQuery(Amount, Description);
        var result = await mediator.Send(operation);
        return result;
    }
